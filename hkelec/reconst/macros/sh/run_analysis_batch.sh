...
[入出力ファイルの仕様]
  ...
  -----------------------------------------------------------------------------
[関連コード・スクリプト (事前にコンパイルが必要)]
  以下のC++プログラムが ${CPP_DIR} に存在し、実行可能である必要があります。
  - fit_pedestal  : ペデスタル解析 (ペデスタル平均・誤差算出)
  - meanfinder    : イベント解析 (電荷計算、時間EMGフィット)
  - plot_summary  : 集計・プロット (グラフ作成、7次関数フィット、CSV出力)
===============================================================================
EOF
}
