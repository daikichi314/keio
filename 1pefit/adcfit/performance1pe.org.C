...
#define inputpath "/data/run/adc"
#define outputpdfpath "./result/"
#define outputtxtpath "./result/"
   
//Run155? used for precalib
   Double_t adcpedestal0[8] = {
     493.863,
     500.922,
     484.62 ,
     425.605,
...
void FitPedPe(TH1* h, const char* name = "", Int_t nPeaks = 3, Int_t rebin = 4, Bool_t DrawEff = kFALSE) {
  ...
  Double_t peakx_pre[3] = {0, 2.4, 4.8};//[pC]
  ...
  h->Fit(funcped,"NQ0","",-1,1);
  //h->Fit(func1pe,"NQ0","", 0.5,3);//h->GetXaxis()->GetXmax());
  h->Fit(func1pe,"NQ0","", 1,peakx_pre[1]*1.5);//h->GetXaxis()->GetXmax());
...
//test
     ...
     if (st0) {
       ...
       myt[2] = new TLatex(0,0,Form("Peak  = %2.2f [pC]",result.peakx));
...
#ifdef __CINT__
int performance1pe(int run = 157, const char* outfilename = "test.root", bool Update = kFALSE, Int_t npeak = 3) {
#else
int main(int argc, char* argv[]){
   if (argc < 3) {
      std::cout << "USAGE : ./performance1pe (Run) (Output Root filename) (num of peak finding) (1:Add in existing root, 0:Recreate new)" << std::endl;
      return 0;
   }
   int run = atoi(argv[1]);
   ...
   TString rootfilename = Form("%s/run%06d.root",inputpath,run);
   TString pdffilename = Form("%s/run%06d_1pefit.pdf",outputpdfpath,run);
   TString textfilename = Form("%s/run%06d_1pefit.txt", outputtxtpath, run);

   //TPaveText *t = new TPaveText(.6,.15,.95,.6);
   //delete pt;
   //t->SetFillStyle(0);
   //t->SetBoaderSize(1);
   //TText *t = 0;
   //t = new TText(0.8,0.8,"ahoaho");
   //t->SetNDC(1);
   ...
   TChain *adcc = new TChain("adc");
   
   TFile* file = new TFile(Form(rootfilename.Data(), run), "READ");
   /// It cannot work.
   //TTree* headert = (TTree*)(file->Get("header"));
   //header hd(headert);
   //  It can work though it's only running on macro.
   //const Int_t nch = 16;
   //const Int_t nread = 7;
   //Double_t HVS[nch];
   //TString serial[nch];
   ...
   //header *hd = new header((TTree*)(file->Get("header")));
         hd.LoadTree(0);
         //hd.fChain->SetBranchStatus("serial",0);  // activate branchname
         hd.fChain->GetEntry(0);
      for (int i = 0; i < (int)hd.serial->size(); i++) {
      ...
      result.LDA = hd.LSamp;
      result.LDW = hd.LSwidth;
   
   adcc->Add(rootfilename.Data());
   ...
   //const char *path = "./result/";
      ...
      //c->Divide(3,3);
   for (int ich = 0; ich < 7/*nread*/; ich++) {
      //c->cd(ich+1);
      //adcc->Draw(Form("(adc.count[%d]-490.8)*50*1e-3*0.1>>h%02d(5000,-16.125,233.825)", ich, ich)); // BL PMT
      adcc->Draw(Form("(adc.count[%d]-%3.3f)*50.*1.e-3*0.1>>h%02d(5000,-16,233.95)", ich, adcpedestal[ich], ich)); // BL PMT
      
      TString serial = (ich<(int)hd.serial->size()?hd.serial->at(ich):"");
      h = (TH1*)(gDirectory->Get(Form("h%02d",ich)));
      h->SetTitle(Form("Ch%d %s %4.1fV Run%05d", ich+1, serial.Data(), hd.HVS[ich], run));
      //h->SetTitle(Form("Ch%d %s %4.1fV Run%05d", ich+1, hd.serial->at(ich).c_str(), hd.HVS[ich], run));
      h->GetXaxis()->SetTitle("Charge [pC]");
      h->GetXaxis()->SetRangeUser(-3,15);
      h->GetXaxis()->SetRangeUser(-0.5,15);
      
      
      //c->SetLogy(1);
      //c->SaveAs(Form("%srun%06d_1pechargeraw_%s.root",path,run,serial.Data()));
      //c->SaveAs(Form("%srun%06d_1pechargeraw_%s.pdf",path,run,serial.Data()));
      
      //FitPedPe(h, Form("BLPMT_%s",serial[ich].Data()), npeak/*Peak*/, 0, kFALSE);
      //std::cout << "BBB RESULT : " << run << " " << ich << " " << serial[ich].Data() << " " << HVS[ich] << " " << gainpeak << " " << respeak << " " << respv << " " << resfwhm << " " << ressigma << std::endl;
      //std::cout << "HVCURVE   CH" << ich << "  " << serial[ich].Data() << ", " << HVS[ich] << ", " << gainpeak << ", " << respeak <<"," << std::endl;
      //sprintf(result.serial, serial[ich].Data());
      //result.ch = ich;
      //result.HVS = HVS[ich];
      FitPedPe(h, Form("BLPMT_%s",serial.Data()), npeak/*Peak*/, 0, kFALSE);
      std::cout << "BBB RESULT : " << run << ", " << ich << ", " << serial.Data() << ", " << hd.HVS[ich] << ", " << gainpeak << ", " << respeak << ", " << respv << ", " << resfwhm << ", " << ressigma << ", " << bsratio << ", " << chi2ndf << ", " << 
(respeak!=0?TMath::Sqrt(resfwhm*resfwhm*respeak*respeak*1e-4-adcpedestalsigma[ich]*50.*1.e-3*0.1*adcpedestalsigma[ich]*50.*1.e-3*0.1)/respeak*100.:-1) << std::endl;
      if (OutputTxt) 
      resultfile << "BBB RESULT : " << run << ", " << ich << ", " << serial.Data() << ", " << hd.HVS[ich] << ", " << gainpeak << ", " << respeak << ", " << respv << ", " << resfwhm << ", " << ressigma << ", " << bsratio << ", " << chi2ndf << ", " << 
(respeak!=0?TMath::Sqrt(resfwhm*resfwhm*respeak*respeak*1e-4-adcpedestalsigma[ich]*50.*1.e-3*0.1*adcpedestalsigma[ich]*50.*1.e-3*0.1*2.355*2.355)/respeak*100.:-1) << std::endl;
      std::cout << "HVCURVE   CH" << ich << "  " << serial.Data() << ", " << hd.HVS[ich] << ", " << gainpeak << ", " << respeak <<"," << std::endl;
      //result.serial = hd.serial->at(ich);
      sprintf(result.serial, serial.Data());//hd.serial->at(ich).c_str())
      result.ch = ich;
      result.HVS = hd.HVS[ich];
...
/*
      t->Clear();
      t->AddText(Form("Gain  : %e", result.gainpeakx)); ((TText*)t->GetListOfLines()->Last())->SetTextColor(kBlue);
      t->AddText(Form("Gain  : %e", result.gainpeakx)); ((TText*)t->GetListOfLines()->Last())->SetTextColor(kBlue);
      t->AddText(Form("P/V   : %2.3f", result.pv));  ((TText*)t->GetListOfLines()->Last())->SetTextColor(kMagenta);
      t->AddText(Form("Sigma : %2.1f%%", ((result.peakx!=0)?result.sigma/result.peakx*100.:0)));  ((TText*)t->GetListOfLines()->Last())->SetTextColor(kRed);
      t->AddText(Form("Sigma : %2.1f%%", ((result.peak1pex!=0)?result.peak1pesigma/result.peak1pex*100.:0)));  ((TText*)t->GetListOfLines()->Last())->SetTextColor(kRed);
      t->Draw();
*/
      ...
      //c->SaveAs(Form("%srun%06d_1pechargelog_p%d_%s.root",path,run,npeak,serial.Data()));
