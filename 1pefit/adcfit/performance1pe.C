...
void FitPedPe(TH1* h, const char* name = "", Int_t nPeaks = 3, Int_t rebin = 4, Bool_t DrawEff = kFALSE) {
  ...
  Double_t peakx_pre[3] = {1500, 1600, 1700};//{0, 2.4, 4.8};//[pC]
...
//#ifdef __CINT__
#ifndef rcomp
void performance1pe(const char* rootfilename = "input.root", const char* outfilename = "test.root", bool Update = kFALSE, Int_t npeak = 3) {
#else
int main(int argc, char* argv[]){
   ...
   for (int ich = 0; ich < 1; ich++) {
      //c->cd(ich+1);
      TCanvas *cfile = (TCanvas*)(file->Get("c"));
      TList *l = (TList*)cfile->GetListOfPrimitives();
      ...
      h->Draw();
      std::cout << "BBB RESULT : " << gainpeak << ", " << respeak << ", " << respv << ", " << resfwhm << ", " << ressigma << ", " << bsratio << ", " << chi2ndf << ", " << 
(respeak!=0?TMath::Sqrt(resfwhm*resfwhm*respeak*respeak*1e-4-adcpedestalsigma[ich]*50.*1.e-3*0.1*adcpedestalsigma[ich]*50.*1.e-3*0.1)/respeak*100.:-1) << std::endl;
      if (OutputTxt) 
      resultfile << "BBB RESULT : " << gainpeak << ", " << respeak << ", " << respv << ", " << resfwhm << ", " << ressigma << ", " << bsratio << ", " << chi2ndf << ", " << 
